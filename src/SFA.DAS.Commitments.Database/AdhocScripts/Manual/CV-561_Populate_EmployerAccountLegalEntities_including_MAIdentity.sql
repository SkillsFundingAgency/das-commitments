...
print 'declare @AccountLegalEntities table ([AccountLegalEntityId] bigint,[AccountLegalEntityPublicHashedId] nvarchar(6),[AccountId] bigint,[Name] nvarchar(100),[Address] NVARCHAR(256),[OrganisationType] TINYINT,[LegalEntityId] NVARCHAR(50),[Created] datetime, [MaLegalEntityId] bigint)'
...
BEGIN TRY
	...
	when 1 then CONVERT(nvarchar(max),'insert into @AccountLegalEntities ([AccountLegalEntityId],[AccountLegalEntityPublicHashedId],[AccountId],[Name],[Address],[OrganisationType],[LegalEntityId],[Created],[MaLegalEntityId]) values') + char(13) + char(10) else '' end +
	CONVERT(NVARCHAR(MAX),' (' + convert(varchar,ale.[Id]) + ', '
		+ '''' + ale.[PublicHashedId] + '''' + ', '
		+ convert(NVARCHAR(MAX),ale.[AccountId]) + ', '
		+ '''' + replace(ISNULL(ale.[Name],''),'''','''''') + '''' + ','
		+ '''' + replace(ISNULL(ale.[Address], ''),'''','''''') + '''' + ','
		+ convert(NVARCHAR(MAX),le.[Source]) + ', '
		+ '''' + replace(ISNULL(le.[Code],''),'''','''''') + '''' + ','
	...
	print '
	insert into AccountLegalEntities([Id],[PublicHashedId],[AccountId],[Name],[Address],[OrganisationType],[LegalEntityId],[Created],[MaLegalEntityId])
	select ale.[AccountLegalEntityId], ale.[AccountLegalEntityPublicHashedId], ale.[AccountId], ale.[Name], ale.[Address],ale.[OrganisationType],ale.[LegalEntityId],ale.[Created],ale.[MaLegalEntityId] 
