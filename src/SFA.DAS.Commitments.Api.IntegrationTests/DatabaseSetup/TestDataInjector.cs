...
namespace SFA.DAS.Commitments.Api.IntegrationTests.DatabaseSetup
{
    public class TestDataInjector
    {
        ...
        public List<DbSetupPriceHistory> PriceHistories { get; }
        ...
        public long NextPriceHistoryId { get; private set; }
        ...
        public static async Task<TestDataInjector> Construct(CommitmentsDatabase commitmentsDatabase, TestIds testIds)
        {
            var testDataInjector = new TestDataInjector
            {
                ...
                NextPriceHistoryId = await commitmentsDatabase.FirstNewId(CommitmentsDatabase.PriceHistoryTableName)
            };
        ...
        public long AddPriceHistory(DbSetupPriceHistory priceHistory, string testIdName = null)
        {
            // id is actually generated by db, but if we insert the entities in order, the ids should match (we can assert that's the case)
            priceHistory.Id = NextPriceHistoryId++;

            PriceHistories.Add(priceHistory);

            if (testIdName != null)
                _testIds[testIdName] = priceHistory.Id;
        ...
        private TestDataInjector()
        {
            ...
            PriceHistories = new List<DbSetupPriceHistory>();
        }
    }
}
