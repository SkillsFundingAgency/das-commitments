...
namespace SFA.DAS.CommitmentsV2.Api.Client.UnitTests.CommitmentsApiClient
{
    ...
    public class WhenCallingTheEndpoints
    {
        ...
        public async Task GetLegalEntity_VerifyUrlAndDataIsCorrectPassedIn()
        {
            ...
            _fixture.MockRestHttpClient.Verify(x=>x.Get<AccountLegalEntity>("api/accountlegalentity/123", null, CancellationToken.None));
        }
        ...
        public async Task CreateCohort_VerifyUrlAndDataIsCorrectPassedIn()
        {
            ...
            _fixture.MockRestHttpClient.Verify(x => x.PostAsJson<CreateCohortRequest, CreateCohortResponse>("api/createcohort", _fixture.CreateCohortRequest, CancellationToken.None));
        }
        [Test]
