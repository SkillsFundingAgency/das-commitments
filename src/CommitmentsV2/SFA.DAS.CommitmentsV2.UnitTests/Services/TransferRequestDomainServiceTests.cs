...
public class TransferRequestDomainServiceTests
{
    ...
    [Test]
    public async Task WhenRejectTransferRequestDirectlyWithPublishEventFalse_ThenDoesNotPublishTransferRequestRejectedEvent()
    {
        _fixture
            .WithTransferRequest(TransferApprovalStatus.Pending);
        ...
        _fixture.TransferRequest.Reject(_fixture.TransferSenderUserInfo, now, publishEvent: false);

        var events = _fixture.UnitOfWorkContext.GetEvents().OfType<TransferRequestRejectedEvent>().ToList();
        Assert.That(events, Has.Count.EqualTo(0));
        Assert.That(_fixture.TransferRequest.Status, Is.EqualTo(TransferApprovalStatus.Rejected));
    }

    [Test]
    public async Task WhenRejectTransferRequestDirectlyWithPublishEventTrue_ThenPublishesTransferRequestRejectedEvent()
    {
        _fixture
            .WithTransferRequest(TransferApprovalStatus.Pending);
        ...
        _fixture.TransferRequest.Reject(_fixture.TransferSenderUserInfo, now, publishEvent: true);

        var events = _fixture.UnitOfWorkContext.GetEvents().OfType<TransferRequestRejectedEvent>().ToList();
        Assert.That(events, Has.Count.EqualTo(1));
        Assert.That(events[0].TransferRequestId, Is.EqualTo(_fixture.TransferRequest.Id));
        Assert.That(events[0].UserInfo, Is.EqualTo(_fixture.TransferSenderUserInfo));
    }

    [Test]
    public async Task WhenRejectTransferRequestDirectlyWithDefaultParameter_ThenPublishesTransferRequestRejectedEvent()
    {
        _fixture
            .WithTransferRequest(TransferApprovalStatus.Pending);
        ...
        _fixture.TransferRequest.Reject(_fixture.TransferSenderUserInfo, now);
...
public class TransferRequestDomainServiceTestsFixture
{
    ...
    public void VerifyTransferApprovalStatusUpdatedToRejected()
    {
        ...
        Assert.That(transferRequest.Status, Is.EqualTo(TransferApprovalStatus.Rejected));
    }
