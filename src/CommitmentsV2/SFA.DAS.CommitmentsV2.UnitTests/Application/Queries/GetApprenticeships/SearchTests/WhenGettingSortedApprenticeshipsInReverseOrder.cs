    ...
    public class WhenGettingSortedApprenticeshipsInReverseOrder : SearchParameterServiceTestBase
    {
            ...
            [Frozen] Mock<ICommitmentsReadOnlyDbContext> mockContext)
        {
           ...
           AssignProviderToApprenticeships(searchParameters.ProviderId ?? 0, apprenticeships);
            ...
            //Assert
            Assert.AreEqual("CC_Should_Be_First_Employer", actual.Apprenticeships.ElementAt(0).Cohort.AccountLegalEntity.Name);
            Assert.AreEqual("BB_Should_Be_Second_Employer", actual.Apprenticeships.ElementAt(1).Cohort.AccountLegalEntity.Name);
            Assert.AreEqual("AA_Should_Be_Third_Employer", actual.Apprenticeships.ElementAt(2).Cohort.AccountLegalEntity.Name);
        }
        ...
        [Test, MoqAutoData]
        public async Task Then_The_Total_Available_Apprentices_Is_Returned_For_Provider(
            ReverseOrderedApprenticeshipSearchParameters searchParameters,
            [Frozen] Mock<ICommitmentsReadOnlyDbContext> mockContext)
        {
            //Arrange
        ...
        [Test, MoqAutoData]
        public async Task Then_The_Total_Available_Apprentices_Is_Returned_For_Employer(
            ReverseOrderedApprenticeshipSearchParameters searchParameters,
            [Frozen] Mock<ICommitmentsReadOnlyDbContext> mockContext)
        {
            searchParameters.FieldName = nameof(Apprenticeship.FirstName);
