...
namespace SFA.DAS.CommitmentsV2.UnitTests.Application.Queries.GetCohortDetails
{
    ...
    public class GetCohortDetailsQueryHandlerTests
    {
        ...
        private async Task CheckCommandResponse(Action<GetCohortDetailsQueryResult> assert)
        {
            // arrange
            var fixtures = new GetCohortDetailsHandlerTestFixtures()
            ...
            // act
            var response = await fixtures.GetResult(new GetCohortDetailsQuery(CohortId));
    ...
    public class GetCohortDetailsHandlerTestFixtures
    {
        public GetCohortDetailsHandlerTestFixtures()
        {
            HandlerMock = new Mock<IRequestHandler<GetCohortDetailsQuery, GetCohortDetailsQueryResult>>();
            ValidatorMock = new Mock<IValidator<GetCohortDetailsQuery>>();
            SeedCohorts = new List<Cohort>();
            EncodingServiceMock = new Mock<IEncodingService>();
        }

        public Mock<IRequestHandler<GetCohortDetailsQuery, GetCohortDetailsQueryResult>> HandlerMock { get; set; }

        public IRequestHandler<GetCohortDetailsQuery, GetCohortDetailsQueryResult> Handler => HandlerMock.Object;

        public Mock<IValidator<GetCohortDetailsQuery>> ValidatorMock { get; set; }
        public IValidator<GetCohortDetailsQuery> Validator => ValidatorMock.Object;

        public List<Cohort> SeedCohorts { get; }
        public Mock<IEncodingService> EncodingServiceMock { get; set; }

        public GetCohortDetailsHandlerTestFixtures AddCommitment(long cohortId, string accountLegalEntityPublicHashedId, long accountLegalEntityId, string legalEntityName, string providerName, EditStatus editStatus, string latestMessageCreatedByEmployer, string latestMessageCreatedByProvider)
        {
        ...
        public Task<GetCohortDetailsQueryResult> GetResult(GetCohortDetailsQuery query)
        {
            return RunWithDbContext(dbContext =>
            {
                var lazy = new Lazy<ProviderCommitmentsDbContext>(dbContext);
                var handler = new GetCohortDetailsQueryHandler(lazy, EncodingServiceMock.Object);
