...
{
    [Test]
    public async Task Handle_WhenAccountLegalEntityHasNotAlreadyBeenDeletedAndAccountProviderLegalEntitiesDoNotExist_ThenShouldNotPublishDeletedPermissionsEvent()
    {
...
public class RemoveAccountLegalEntityCommandHandlerTestsFixture : IDisposable
{
    ...
    public RemoveAccountLegalEntityCommandHandlerTestsFixture()
    {
        ...
        AccountLegalEntity = ObjectActivator.CreateInstance<AccountLegalEntity>()
            .Set(ale => ale.Id, 2)
    ...
    {
        AccountLegalEntity.Set(ale => ale.Deleted, Command.Removed.Value.AddHours(-1));
    ...
    public void VerifyCohortIsMarkedAsDeletedAndEventIsEmitted()
    {
        ...
        {
            Cohort.IsDeleted.Should().BeTrue("Cohort is not marked as deleted");
            emittedEvent.CohortId.Should().Be(Cohort.Id);
            emittedEvent.AccountId.Should().Be(Cohort.EmployerAccountId);
            emittedEvent.ProviderId.Should().Be(Cohort.ProviderId);
        }
    }
    ...
    public void VerifyDraftApprenticeshipDeletedAndEventEmitted()
    {
        ...
        deleted.Should().BeNull("Draft apprenticeship record not deleted");
