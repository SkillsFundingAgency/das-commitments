...
public class GetDraftApprenticeshipCreatedEventsForCohortQueryHandler(Lazy<ProviderCommitmentsDbContext> db) : IRequestHandler<GetDraftApprenticeshipCreatedEventsForCohortQuery, GetDraftApprenticeshipCreatedEventsForCohortQueryResult>
{
    public async Task<GetDraftApprenticeshipCreatedEventsForCohortQueryResult> Handle(GetDraftApprenticeshipCreatedEventsForCohortQuery command, CancellationToken cancellationToken)
    {
        var cohort = await db.Value.Cohorts
            ...
            .SingleAsync(x => x.Id == command.CohortId, cancellationToken)
        ...
        return new GetDraftApprenticeshipCreatedEventsForCohortQueryResult(
            cohort.Apprenticeships.Select(x => new DraftApprenticeshipCreatedEvent(
                x.Id,
                command.CohortId,
                x.Uln,
                x.ReservationId,
