...
public class GetLastSubmissionEventIdQueryHandler : IRequestHandler<GetLastSubmissionEventIdQuery, long?>
{
    ...
    {
        var result = await _dbContext.Value.JobProgress.Select(x => x.AddEpaLastSubmissionEventId).FirstOrDefaultAsync(cancellationToken: cancellationToken);
        _logger.LogInformation("Last SubmissionEventId processed by previous job run is {Result}", result ?? 0);
