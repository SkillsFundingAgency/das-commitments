...
public class ApprenticeshipSearchService(IProviderCommitmentsDbContext dbContext) : IApprenticeshipSearchService<ApprenticeshipSearchParameters>
{
        ...
        if (searchParameters.ReverseSort)
        {
            if (skipCount < totalApprenticeshipsWithoutAlerts)
            {
                var apprenticeshipsWithoutAlerts =
                    await GetApprenticeshipsWithoutAlerts(searchParameters, skipCount, searchParameters.CancellationToken);
            ...
            var apprenticeshipsWithAlerts = await GetApprenticeshipsWithAlerts(searchParameters, skipCount, searchParameters.CancellationToken);
        ...
        else
        {
            if (skipCount < totalApprenticeshipsWithAlerts)
            {
                var apprenticeshipsWithAlerts =
                    await GetApprenticeshipsWithAlerts(searchParameters, skipCount, searchParameters.CancellationToken);
            ...
            var apprenticeshipsWithoutAlerts = await GetApprenticeshipsWithoutAlerts(searchParameters, skipCount, searchParameters.CancellationToken);
    ...
    private async Task<List<Apprenticeship>> GetApprenticeshipsWithoutAlerts(ApprenticeshipSearchParameters searchParameters, int skipCount, CancellationToken cancellationToken)
    {
