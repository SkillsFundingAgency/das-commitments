...
namespace SFA.DAS.CommitmentsV2.Application.Queries.GetCohortEmailOverlaps
{
    public class GetCohortEmailOverlapsQueryHandler : IRequestHandler<GetCohortEmailOverlapsQuery, GetCohortEmailOverlapsQueryResult>
    {
        ...
        public GetCohortEmailOverlapsQueryHandler(IOverlapCheckService overlapCheckService)
        {
        ...
        private GetCohortEmailOverlapsQueryResult Map(List<EmailOverlapCheckResult> results)
        {
            var response = new GetCohortEmailOverlapsQueryResult();
            response.Overlaps = results.Select(x => new ApprenticeshipEmailOverlap
