...
public class OverlappingTrainingDateRequestNotificationToServiceDeskCommandHandler : IRequestHandler<OverlappingTrainingDateRequestNotificationToServiceDeskCommand>
{
    ...
    public async Task Handle(OverlappingTrainingDateRequestNotificationToServiceDeskCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation("OLTD_GoLiveDate {GoLiveDate}", _configuration.OLTD_GoLiveDate.Value.ToString());
        }
        else
        {
        ...
        var pendingRecords = _dbContext.Value.OverlappingTrainingDateRequests
            ...
            .Where(overlappingTrainingDateRequest => overlappingTrainingDateRequest.NotifiedServiceDeskOn == null
                                                     ...
                                                     && (overlappingTrainingDateRequest.CreatedOn < goLiveDate
                                                         ? overlappingTrainingDateRequest.CreatedOn < currentDate.AddDays(-28).Date
                                                         : overlappingTrainingDateRequest.CreatedOn < currentDate.AddDays(-14).Date))
            .ToList();

        _logger.LogInformation("Found {PendingRecordsCount} records which need overlapping training reminder for Service Desk", pendingRecords.Count);
