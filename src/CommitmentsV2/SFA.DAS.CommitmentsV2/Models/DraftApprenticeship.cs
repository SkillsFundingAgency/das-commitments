...
public class DraftApprenticeship : ApprenticeshipBase, ITrackableEntity
{
        ...
        int minimumOffTheJobTrainingHoursRequired)
    {
        ...
        void CheckHoursReductionIsSensible(List<DomainError> errors1)
        {
            ...
            else if (trainingTotalHours - durationReducedByHours < minimumOffTheJobTrainingHoursRequired)
            {
                errors1.Add(new DomainError("DurationReducedByHours",
                    $"The remaining off-the-job training is below the minimum {minimumOffTheJobTrainingHoursRequired} hours required for funding. Check if the RPL reduction is too high"));
            }
        }
        ...
        {
            if (trainingTotalHours.Value < minimumOffTheJobTrainingHoursRequired)
            {
                domainErrors.Add(new DomainError("trainingTotalHours",
                    $"Total off-the-job training time for this apprenticeship standard must be {minimumOffTheJobTrainingHoursRequired} hours or more"));
            }
