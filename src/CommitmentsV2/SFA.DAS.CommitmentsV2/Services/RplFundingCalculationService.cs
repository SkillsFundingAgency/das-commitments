...
public class RplFundingCalculationService : IRplFundingCalculationService
{
    ...
    private static async Task<int?> GetFundingBandMaximum(string courseCode, DateTime? startDate, DbSet<FrameworkFundingPeriod> frameworkFundingPeriods, DbSet<StandardFundingPeriod> standardFundingPeriods)
    {
        ...
        var framework = await frameworkFundingPeriods
            .Where(c => c.Id.Equals(courseCode) && c.EffectiveFrom <= startDate && (c.EffectiveTo == null || c.EffectiveTo >= startDate))
            .OrderByDescending(x => x.EffectiveFrom).FirstOrDefaultAsync();
