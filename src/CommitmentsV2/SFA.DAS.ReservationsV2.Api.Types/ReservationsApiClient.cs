...
namespace SFA.DAS.Reservations.Api.Types
{
    public class ReservationsApiClient : IReservationsApiClient
    {
        ...
        public Task<CreateAutoReservationResponse> CreateAutoReservation(CreateAutoReservationRequest request, CancellationToken cancellationToken)
        {
            var url = BuildUrl($"api/accounts/{request.AccountId}/reservations");
            return _httpHelper.PostAsJson<CreateAutoReservationRequest, CreateAutoReservationResponse>(url, request, cancellationToken);
        }
